import requests
from dreamcraft.config import LOG_DIR
from dreamcraft.domain.observation import Observation
...
class MinecraftClient():
    ...
    def _start_mineflayer(self, options) -> Observation: 
        retry = 0
    ...
    def start(self, options) -> Observation:
        self.reset_options = {
            "port": self.mc_port,
        ...
        self._start_azure()
        obs = self._start_mineflayer(options)
        if not obs:
            obs = self.observe()
        self.connected = True
        return obs
    ...
    def execute(
        ...
        return {
            ...
            "observation": Observation.model_validate_json(res.json()) if res.status_code == 200 else None,
