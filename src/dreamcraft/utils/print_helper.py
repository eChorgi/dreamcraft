...
def ipynb_print(data, include:list[str]=None, exclude:list[str]=None):
    def _parse_node(node, level=0, layers = None):
        if include is not None and layers and layers[-1] not in include:
            return ""
        if exclude is not None and layers and layers[-1] in exclude:
            return ""
        md = ""
        ...
        if isinstance(node, dict):
            ...
            for i, item in enumerate(node):
                if level == 0:
                    ...
                    # 嵌套列表：使用柔和的浅色序号标识
                    md += f"{indent}- <span style='color:#9bbcd2'>{layers[-1]}[{i}]</span>:\n"
                    md += _parse_node(item, level + 1, layers + [''])
        elif hasattr(node, "__dict__"):
            # 获取类的名称，作为装饰性的 Key
            class_name = node.__class__.__name__
            md += f"{indent}- **<span style='color:#e67e22'>{class_name}</span>**:\n"
            # 递归解析该对象的成员变量
            md += _parse_node(node.__dict__, level + 1, layers + [class_name])     
        else:
            # 递归的尽头：基本数据类型 (字符串、数字、布尔值等)
            if layers[-1]:
                key_format = f"<span style='color:#6a8a9f'>**{layers[-1]}**</span>"
            else:
                key_format = ""
            str_node = str(node)
            ...
            if is_multiline:
                ...
                # 单行文本：紧凑显示
                md += f"{indent}- {key_format} : `{str_node.replace('`', '"')}`\n"
